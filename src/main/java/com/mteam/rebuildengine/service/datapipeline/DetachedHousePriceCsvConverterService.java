...
// FEATURE_16_PRICE_DATA_MIGRATION.md §5.1 — 개별주택가격정보(단독주택·다가구주택 공시가격, V-WORLD,
// CP949, 서울만)를 적재용 스키마로 정규화한다. land_price(AL_D151)와 원본 발급 체계가 같아
// LandPriceCsvConverterService와 동일한 구조(고유번호/법정동코드/법정동명/지번/기준연도/기준월/…)를
// 쓰되, 원본에 건축물대장고유번호·건물면적 등 추가 컬럼이 섞여 있어 컬럼 인덱스만 다르다.
@Service
public class DetachedHousePriceCsvConverterService {

    private static final Logger logger = LogManager.getLogger(DetachedHousePriceCsvConverterService.class);
    private static final int BUFFER_SIZE = 1 << 16;

    // 원본 컬럼 인덱스(0-based, 실측 확인) — 고유번호,법정동코드,법정동명,특수지구분코드,특수지구분명,
    // 지번,건축물대장고유번호,기준연도,기준월,동코드,동명,건물산정연면적,토지대장면적,산정대지면적,
    // 건물전체연면적,주택가격,표준지여부,데이터기준일자,원천시도시군구코드
    private static final int COL_BJDONG_NM = 2;
    private static final int COL_LOT_NO = 5;
    private static final int COL_BASE_YEAR = 7;
    private static final int COL_BASE_MONTH = 8;
    private static final int COL_PRICE = 15;
    ...
    public ConvertResult convert() throws IOException {
        File rawFile = new File(dataDir, "raw/detached_house_price/AL_D169_11_20260526.csv");
        File outputFile = new File(dataDir, "converted/detached_house_price_utf8.csv");
        ...
        logger.info("개별주택가격정보 변환 완료: {}건 -> {}", total, outputFile);
    ...
    // "서울특별시 강동구 천호동" -> ["서울특별시 강동구", "천호동"] — land_price(F-16)와 동일 방식.
    ...
    // "465-22" -> ["0465", "0022"], "591"(부번 없음) -> ["0591", "0000"]. building.mn_lotno/sub_lotno
    // (F-13)가 4자리 0패딩이라 여기서 맞춘다(land_price와 동일, FEATURE_16 §3.3).
