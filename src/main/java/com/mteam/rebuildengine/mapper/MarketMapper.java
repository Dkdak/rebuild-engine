...
public interface MarketMapper {
    // F-08 §3.4-B/§3.5 — 조건에 맞는 유사 거래의 ㎡당 가격 p25/중앙값/p75·건수를 한 번에 집계.
    ...
    // §8.17 "시장 내 가격 위치" thisPropertyPercentile — comparableTradeCondition과 같은 모집단 안에서
    // condition.value() 이하인 거래 비율(0~100). 모집단이 비어있으면 null.
    BigDecimal findComparableTradeRank(PricePositionRankCondition condition);

    // §8.17 "거래 활성도" — SAME_DONG 단계와 같은 필터로 최근 1/3/5년 거래건수를 한 쿼리로 집계.
    TradeActivityReadModel findTradeActivityCounts(TradeActivityCondition condition);
}
